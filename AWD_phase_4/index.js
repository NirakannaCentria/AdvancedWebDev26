...
const resourceValidators = [
  ...
  .matches(/^[^<>]*$/).withMessage('resourceDescription contains invalid characters'), //added
  ...
  .isFloat({ gt: 0 }).withMessage('resourcePrice must be a positive number')
  .toFloat(),
  ...
  body('resourcePriceUnit')
    ...
    .trim()
    .isIn(['hour', 'day', 'week', 'month']) //added week and month
...
app.post('/api/resources', resourceValidators, async (req, res) => {
 ...
 const {
  ...
  resourcePriceUnit
    ...
    //const params = [
      //crypto.createHash('sha256').update(resourceName, 'utf8').digest('hex'), (removed)
     // resourceName, //(added)
     // resourceDescription,
   //   Boolean(resourceAvailable),
      //Number(resourcePrice)*2, (removed)
    //  Number(resourcePrice), //(added)
     // resourcePriceUnit];
