...
router.post("/", resourceValidators, async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    await logEvent({
      actorUserId: null,
      message: `RESOURCE_CREATE_BLOCKED_VALIDATION: ${validationDetails} (400)`,
      entityType: "resource",
      entityId: null,
    });
  ...
  try {
    ...
    const resourceId = rows[0].id;
        await logEvent({
      actorUserId,
      message: `RESOURCE_CREATED: id=${resourceId}, name="${resourceName}", price=${Number(resourcePrice)}, unit="${resourcePriceUnit}"`,
      entityType: "resource",
      entityId: resourceId,
    });
