...
function buildValidationMessage(errors) {
  if (!Array.isArray(errors) || errors.length === 0) {
    return "Could not add resource because some input is invalid. Check the form fields, correct the values, and try again.";
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // 409 = duplicate resourceName (our new feature)
        ...
        showFormMessage(
          "info",
          "This resource already exists. Please change the name or check the resource list before trying again."
        );
        return;
      }
    ...
    showFormMessage(
      "success",
      `Resource "${resourceName}" was added successfully. You can now see it in the resource list.`
    );
